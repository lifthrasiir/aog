...
For the purpose of this puzzle, the input format is a concatenation of a list of global puzzle rules, a visual grid and a list of edge or vertex clues. The visual grid is a possibly incomplete lattice which columns are visually aligned; `+` (or any vertex clue symbol) denotes a vertex, `|` (or any edge clue string) denotes a vertical pre-cut edge, and a sequence of `-`s (or any edge clue string with optional `-`s around) denotes a horizontal pre-cut edge. Any uncut edge is denoted by a single dot (`.`) in either horizontal or vertical edges. Any cell without a clue would be denoted by a single underscore (`_`). Since columns are aligned, leading spaces are significant and have to be considered when interpreting the grid. (Note that the output uses a slightly different convention for visual clarity: a dot is omitted and all cells are labelled with numbers which are unique to each shape. So it is technically possible to rely only on shape numbers and completely ignore grids.)
