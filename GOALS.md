...
- compass (cEAST,WEST,SOUTH,NORTH; empty for missing directions): Relative to the clued cell, gives the number of cells within the same piece in the east/west (column higher/lower) and south/north (row higher/lower) directions. One or more directions may be blank, which means no information is given (not zero cells---if zero, it would be written as 0). An entirely empty compass (c,,,) can exist and still counts as a clue for the solitude rule.
